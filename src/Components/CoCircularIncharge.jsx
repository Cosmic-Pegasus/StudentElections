...
const CoCuricularIncharge = () => {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        ...
        setBoysCandidates(response.data.boys['Co-Curricular Incharge']);
        setGirlsCandidates(response.data.girls['Co-Curricular Incharge']);
      } catch (error) {
    ...
    if (selectedCandidate && selectedSection) {
      try {
        await axios.post('http://localhost:5000/vote', {
          ...
          position: 'Co-Curricular Incharge',
  ...
  return (
    <div className='flex flex-col items-center justify-center gap-10'>
      <h2 className='raleway-bold text-5xl section-heading text-animation'>Co-Curricular Incharge</h2>
      {!confirmation && (
        <>
...
export default CoCuricularIncharge;
